...
Java_com_tcl_sentenecepiece_SpmNative_loadModel(JNIEnv* env, jobject /*thiz*/, jstring path_) {
...
Java_com_tcl_sentenecepiece_SpmNative_encode(JNIEnv* env, jobject /*thiz*/, jstring text_) {
...
Java_com_tcl_sentenecepiece_SpmNative_encodeToIds(JNIEnv* env, jobject /*thiz*/, jstring text_) {
	const char* text = env->GetStringUTFChars(text_, nullptr);
	std::vector<int> ids;
...
Java_com_tcl_sentenecepiece_SpmNative_decode(JNIEnv* env, jobject /*thiz*/, jobjectArray pieces_) {
	if (pieces_ == nullptr) {
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_tcl_sentenecepiece_SpmNative_decodeFromIds(JNIEnv* env, jobject /*thiz*/, jintArray ids_) {
	if (ids_ == nullptr) {
		return env->NewStringUTF("");
	}
